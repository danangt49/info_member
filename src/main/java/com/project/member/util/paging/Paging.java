...
public class Paging {
  ...
  public static Paging of(int totalPages, int pageNumber, int pageSize, long totalElements) {
    ...
    if (totalPages < PAGINATION_STEP * 2 + 6) {
      ...
      paging.first(pageNumber);
      paging.addPageItems(
          pageNumber - PAGINATION_STEP, pageNumber + PAGINATION_STEP + 1, pageNumber);
      paging.last(totalPages);
    }
  ...
  public void last(int pageSize) {
    ...
    items.add(
        PageItem.builder().active(true).index(pageSize).pageItemType(PageItemType.PAGE).build());
  }
